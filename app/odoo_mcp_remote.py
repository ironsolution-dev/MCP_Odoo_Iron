...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "(1) Claude.ai descubre las 38 tools nativas (odoo_my_tasks, "
        "odoo_create_project_task_apl, etc.) — usalas directamente. "
        "(2) ChatGPT chat-mode solo descubre `search(query)` y `fetch(id)`. "
        "PARA LEER en ChatGPT: search('mis tareas'), search('proyectos'), "
        "search('empleados'), search('contactos'). Despues fetch('task:42') "
        "para detalle. "
        "PARA ESCRIBIR en ChatGPT: envia search() con un JSON action dentro "
        "del query. Formato: search('{\"action\":\"create_task\",\"project_id\":3,"
        "\"title\":\"[APL 2.0][P2][Area][Tipo] Verbo + ...\",\"description\":\""
        "Objetivo: ...\\\\nEntregable: ...\\\\nResponsable: ...\\\\nFecha limite: "
        "...\\\\nCriterio de cierre: ...\\\\nEvidencia requerida: ...\\\\nRiesgo "
        "si no se cierra: ...\\\\nSiguiente accion: ...\",\"deadline\":\""
        "YYYY-MM-DD\",\"area\":\"...\",\"task_type\":\"...\",\"priority\":\"P2\"}'). "
        "Acciones validas: create_task, create_todo, update_task, move_task, "
        "close_task, cancel_task, create_project, create_event. "
        "Si llamas search con verbos de escritura SIN JSON, te devolvere un "
        "template del formato — usalo como referencia y vuelve a llamar con JSON. "
        "Toda escritura sigue APL 2.0 (6 campos obligatorios). "
        "No inventes datos: si la tool retorna vacio, reporta vacio."
    ),
    stateless_http=True,
    json_response=True,
    host='0.0.0.0',
